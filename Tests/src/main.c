...
int main (void)
{
  ...
  IrFramePolling.Tr = 25.0f - 8.0f; // Set the first Tr at 25-8А. Next will be get from the last calculated frame
#else
  float Tr = 25.0f - 8.0f; // Set the first Tr at 25-8А. Next will be get from the last calculated frame
#endif
...
//      Error = MLX90640_ExtractDeviceParameters(IrCAM, &IrCAM_EEPROM, false);  // Do not dump EEPROM
  ...
  while(1)
  {
    ...
    if (Error == ERR_OK)
    {
      IrFramePolling.Tr = IrFramePolling.Result.Ta - 8.0f; // Calculate next Tr: Tr = Ta - 8А
    }
